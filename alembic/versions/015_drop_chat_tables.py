...
The chat system (conversations, messages, and related tables) has been
removed from the codebase — all chat endpoints, services, models, and the
websocket manager are deleted. This migration drops the underlying tables
rather than leaving them as orphaned schema.

001_add_chat_tables and 005_fix_media_type_varchar are NOT deleted or
renumbered — they remain in history as the (now superseded) record of how
these tables were created and altered. upgrade() here mirrors 001's own
downgrade() (same drop order); downgrade() mirrors 001's upgrade() (same
...
revision = "015_drop_chat_tables"
down_revision = "014_add_inbox_items"
branch_labels = None
depends_on = None
...
def downgrade() -> None:
    # Each sa.Enum(...) below implicitly issues its own CREATE TYPE the first
    # time it's used in a create_table call — no separate pre-creation needed
    # (doing both, as an earlier draft of this migration did, fails with
    # "type already exists" since neither statement is conditional together).
    op.create_table(
        "conversations",
