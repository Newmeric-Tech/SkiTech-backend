...
# ===========================================================
# EMPLOYEE SCHEDULING (Staff Dashboard & Manager Scheduling)
# ===========================================================

class EmployeeAvailability(Base, UUIDMixin, TimestampMixin):
    """Track employee availability per day (off-days, leaves, etc.)"""
    ...
    status = Column(String(50), nullable=False)  # available / off / leave / holiday / sick
...
class WeeklySchedule(Base, UUIDMixin, TimestampMixin):
    """Weekly schedules for employees with shift timings and assignments"""
    ...
    status = Column(String(50), nullable=False, default="draft")  # draft / assigned / published / completed
    ...
    __table_args__ = (
        ...
        Index("idx_schedule_property", "property_id"),
        Index("idx_schedule_week", "week_start_date", "week_end_date"),
...
class ShiftAssignment(Base, UUIDMixin, TimestampMixin):
    """Individual shift assignments for each day of the weekly schedule"""
    __tablename__ = "shift_assignments"

    tenant_id = Column(UUID(as_uuid=True), ForeignKey("tenants.id", ondelete="CASCADE"), nullable=False)
    property_id = Column(UUID(as_uuid=True), ForeignKey("properties.id", ondelete="CASCADE"), nullable=False)
    schedule_id = Column(UUID(as_uuid=True), ForeignKey("weekly_schedules.id", ondelete="CASCADE"), nullable=False)
    employee_id = Column(UUID(as_uuid=True), ForeignKey("employees.id", ondelete="CASCADE"), nullable=False)

    shift_date = Column(DateTime, nullable=False)
    shift_start_time = Column(String(5), nullable=False)  # HH:MM format
    shift_end_time = Column(String(5), nullable=False)
    shift_type = Column(String(50))  # morning / afternoon / night / full-day
    status = Column(String(50), nullable=False, default="scheduled")  # scheduled / off / conflict / covered

    __table_args__ = (
        Index("idx_shift_assignment_employee", "employee_id"),
        Index("idx_shift_assignment_schedule", "schedule_id"),
        Index("idx_shift_assignment_date", "shift_date"),
...
class ReplacementRequest(Base, UUIDMixin, TimestampMixin):
    """When an employee is off/unavailable, manager can create replacement requests"""
    ...
    original_employee_id = Column(UUID(as_uuid=True), ForeignKey("employees.id", ondelete="CASCADE"), nullable=False)
    replacement_employee_id = Column(UUID(as_uuid=True), ForeignKey("employees.id", ondelete="SET NULL"), nullable=True)
    ...
    request_type = Column(String(50), nullable=False)  # direct_assignment / send_request
    status = Column(String(50), nullable=False, default="pending")  # pending / accepted / rejected / assigned / cancelled
    ...
    __table_args__ = (
        Index("idx_replacement_original_employee", "original_employee_id"),
        Index("idx_replacement_employee", "replacement_employee_id"),
        Index("idx_replacement_status", "status"),
        Index("idx_replacement_shift_date", "shift_date"),
        Index("idx_replacement_priority", "priority"),
    )
...
class ShiftResponse(Base, UUIDMixin, TimestampMixin):
    """Track employee responses to shift requests"""
    __tablename__ = "shift_responses"

    tenant_id = Column(UUID(as_uuid=True), ForeignKey("tenants.id", ondelete="CASCADE"), nullable=False)
    property_id = Column(UUID(as_uuid=True), ForeignKey("properties.id", ondelete="CASCADE"), nullable=False)
    replacement_request_id = Column(UUID(as_uuid=True), ForeignKey("replacement_requests.id", ondelete="CASCADE"), nullable=False)

    employee_id = Column(UUID(as_uuid=True), ForeignKey("employees.id", ondelete="CASCADE"), nullable=False)
    response_type = Column(String(50), nullable=False)  # accept / reject
    reason = Column(Text)
    ...
    __table_args__ = (
        ...
        Index("idx_shift_response_employee", "employee_id"),
        Index("idx_shift_response_request", "replacement_request_id"),
    )

    replacement_request = relationship("ReplacementRequest")
    employee = relationship("Employee")


class EmployeeSkill(Base, UUIDMixin, TimestampMixin):
    """Track employee skills for AI recommendations"""
    ...
    __table_args__ = (
        ...
        Index("idx_employee_skills_skill", "skill_name"),
    )
...
# ===========================================================
# ERROR & COMPLAINT LOG (Manager, Owner, Staff Dashboards)
# ===========================================================

class Complaint(Base, UUIDMixin, TimestampMixin, SoftDeleteMixin):
    """Main complaint model for tracking errors and complaints"""
    ...
    created_by = Column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), nullable=True)  # Staff/Employee who reported
    assigned_to = Column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), nullable=True)  # Staff/Manager assigned
    assigned_by = Column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), nullable=True)  # Who assigned it
    
    # Complaint details
    title = Column(String(255), nullable=False)
    description = Column(Text, nullable=False)
    category = Column(String(100), nullable=False)  # MAINTENANCE, HOUSEKEEPING, TECHNICAL, etc.
    complaint_type = Column(String(50), nullable=False)  # Complaint / Error / Handover
    
    # Priority and status
    priority = Column(String(20), nullable=False, default="medium")  # low / medium / high / critical
    status = Column(String(50), nullable=False, default="open")  # open / in_progress / resolved / escalated / closed
    ...
    __table_args__ = (
        ...
        Index("idx_complaint_resolved_at", "resolved_at"),
    )
    ...
    assigner = relationship("User", foreign_keys=[assigned_by], viewonly=True)
    resolver = relationship("User", foreign_keys=[resolved_by], viewonly=True)
...
class ComplaintComment(Base, UUIDMixin, TimestampMixin):
    """Comments and activity log for complaints"""
    ...
    is_internal = Column(Boolean, default=False)  # Only visible to managers/admins
    
    # Metadata
    attachment_count = Column(Integer, default=0)

    __table_args__ = (
        Index("idx_complaint_comment_complaint", "complaint_id"),
        Index("idx_complaint_comment_user", "user_id"),
        Index("idx_complaint_comment_created", "created_at"),
    )
...
class ComplaintAttachment(Base, UUIDMixin, TimestampMixin):
    ...
    file_path = Column(String(500), nullable=False)  # S3 or storage path
...
class ComplaintCommentAttachment(Base, UUIDMixin, TimestampMixin):
    """Attachments for complaint comments"""
    __tablename__ = "complaint_comment_attachments"

    tenant_id = Column(UUID(as_uuid=True), ForeignKey("tenants.id", ondelete="CASCADE"), nullable=False)
    comment_id = Column(UUID(as_uuid=True), ForeignKey("complaint_comments.id", ondelete="CASCADE"), nullable=False)
    uploaded_by = Column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
    ...
    __table_args__ = (
        Index("idx_comment_attachment_comment", "comment_id"),
        Index("idx_comment_attachment_uploaded_by", "uploaded_by"),
    )
