...
class AttendanceService:
    ...
    @staticmethod
    def get_active_punch_in(db: Session, user_id: str, property_id: str, tenant_id: str) -> Optional[AttendanceRecord]:
    ...
    def get_attendance_history(
        ...
        query = db.query(AttendanceRecord).filter(
            and_(AttendanceRecord.user_id == user_id, AttendanceRecord.tenant_id == tenant_id)
        )
        if property_id:
        ...
        records = query.order_by(desc(AttendanceRecord.punch_in_time)).offset(skip).limit(limit).all()
        return records, total_count

    @staticmethod
    def get_daily_summary(db: Session, user_id: str, tenant_id: str, date: datetime) -> dict:
...
class GeofenceService:
    @staticmethod
    def create_geofence(db: Session, tenant_id: str, geofence_data: PropertyGeofenceCreate) -> PropertyGeofence:
    ...
    @staticmethod
    def update_geofence(db: Session, geofence_id: str, geofence_data: PropertyGeofenceCreate) -> PropertyGeofence:
        geofence = db.query(PropertyGeofence).filter(PropertyGeofence.id == geofence_id).first()
        if not geofence:
    ...
    @staticmethod
    def get_geofence(db: Session, property_id: str, tenant_id: str) -> Optional[PropertyGeofence]:
    ...
    @staticmethod
    def list_geofences(db: Session, tenant_id: str, skip: int = 0, limit: int = 100) -> Tuple[List[PropertyGeofence], int]:
        query = db.query(PropertyGeofence).filter(PropertyGeofence.tenant_id == tenant_id)
